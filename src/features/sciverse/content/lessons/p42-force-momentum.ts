...
export function getP42Script(): Record<string, DialogNode> {
    return {
        root: {
            id: 'root',
            speaker: 'AI',
            content: "Coaches always shout the same thing: **follow through**. Do not stop your swing at the ball -- swing the bat right through it.\n\nBut the ball has already gone by then. How can what your bat does *afterwards* possibly matter?",
            onEnterAction: { type: 'SET_VISUAL', payload: { phase: 'intro' } },
            options: [
                { id: 'good', label: "Following through keeps the bat pushing on the ball for longer, and a longer push means a faster ball.", nextNodeId: 'correct', sentiment: 'positive' },
        ...
        misconception: {
            id: 'misconception',
            speaker: 'AI',
            content: "It looks like the ball leaves instantly, but it does not! A **bat** stays in contact with a ball for about **1 millisecond**. (A foot kicking a football is slower -- about **10 milliseconds**.)\n\nThat sounds like nothing, but during that tiny window the ball is being pushed. **Follow through** keeps the push going for the whole contact instead of cutting it short.\n\nIf you stop your swing at the ball, the bat starts slowing down *before* contact ends. The ball gets a shorter, weaker push.",
            options: [
        ...
        checkpoint: {
            id: 'checkpoint',
            speaker: 'AI',
            content: "**Checkpoint:** Two players hit a ball with exactly the same force. One stabs at it and pulls the bat back. The other follows through smoothly.\n\nWhose ball travels faster?",
            options: [
