...
export function getL2C33Script(): Record<string, DialogNode> {
    return {
        ...
        budget: {
            id: 'budget',
            speaker: 'AI',
            content: "Now scale that up from one tree to a whole landscape.\n\nBefore any of this means anything you have to say **which reservoir you are tracking**, because \"in\" and \"out\" only mean something once there is a thing to be in or out of.\n\nThe lab is **tracking** one reservoir: **plants and soil**. So every number is either carbon going **into plants and soil**, or carbon coming **out of plants and soil**. The same flux would be an \"out\" if you had chosen to track the air instead.\n\nEcologists measure both fluxes in **kilograms of carbon per square metre per year**:\n\n**Flux in** -- what photosynthesis moves from the air into the plants.\n**Flux out** -- what respiration and the **decomposers** move back into the air.\n\nNeither number on its own tells you whether carbon is building up. Only the difference does.\n\nSubtract, and the sign tells you what kind of place you are standing in. Two words are used for this, and their names trip almost everyone up the first time, so let us take them slowly.\n\n- **In is bigger than out** -- carbon is piling up in the land. The land is a **carbon sink**.\n- **Out is bigger than in** -- stored carbon is leaving the land. The land is a **carbon source**.\n- **In equals out** -- nothing accumulates. This is **steady state**, and most old forests sit close to it.\n\n**If that feels backwards, you have spotted something real.** You have been watching the plants and soil, and the **sink** is the one where that store is *filling up*. Surely a thing that is filling up should be called the source?\n\nThe answer is that **these two words are not describing the land at all. They describe what is happening to the air.**\n\nThink of a kitchen sink. Water goes down it and **leaves the room**. A carbon **sink** is where carbon leaves the **air** -- it disappears from the sky into the ground. The land is gaining, but the words are told from the air's point of view, and from up there the carbon has gone.\n\nA **source** is where something comes **from**, like the source of a river. A carbon source is where carbon comes from, back **into** the air.\n\nSo whenever the words feel upside down, ask them about the **air** instead of the ground:\n\n- **Carbon sink** -- the air is **losing** carbon; the land is gaining it\n- **Carbon source** -- the air is **gaining** carbon; the land is losing it\n\nThat is also why the words matter so much in the news. When people worry about a forest \"becoming a carbon source\", they mean the air is about to start gaining what that forest had been holding.\n\nAnd because it is a subtraction rather than a multiplication, the total just adds up year after year:\n\n**carbon stored after n years = net change x n**\n\nSlide **Photosynthesis** and **Decomposition** and watch the balance tip.",
